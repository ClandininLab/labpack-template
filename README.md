...
| `template_labpack/daq.py` | DAQ drivers (NI, LabJack) and the `DAQonServer` proxy used when the DAQ lives on the rig machine. Referenced by the `trigger:` string in a rig config. |
| `template_labpack/dlpc350.py` | TI DLPC350 projectors (LightCrafter 4500 and the optical engines built on it). Sets pattern mode, LED currents, and can display the three color channels of a frame as successive patterns — a 120 Hz link driving the DMD at 360 Hz. |
